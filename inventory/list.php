...
$hasStockCol = columnExists($pdo, 'medicine_batches', 'stock_on_hand');
$ttCol = $hasStockCol ? null : (columnExists($pdo, 'stock_transactions', 'transaction_type') ? 'transaction_type' : 'type');

if ($hasStockCol) {
    // Main inventory (not expired, not near expiry)
    $sql = "
        SELECT
            b.id AS batch_id,
            m.id AS medicine_id,
    ...
    $nearSql = "
        SELECT
            ...
            b.batch_no, b.expiry_date, b.stock_on_hand AS stock
        FROM medicine_batches b
        JOIN medicines m ON m.id = b.medicine_id
    ...
    $sql = "
        SELECT
            ...
            b.batch_no,
            b.expiry_date,
            COALESCE(SUM(CASE WHEN t.`$ttCol` = 'IN' THEN t.quantity ELSE -t.quantity END), 0) AS stock
        FROM medicine_batches b
        JOIN medicines m ON m.id = b.medicine_id
